...
class TestCreateBarbellLinks:
    ...
    def test_creates_three_links(self, robot: Any) -> None:
        spec = BarbellSpec.mens_olympic()
    ...
    def test_creates_two_fixed_joints(self, robot: Any) -> None:
        spec = BarbellSpec.mens_olympic()
        create_barbell_links(robot, spec)
        joints = robot.findall("joint[@type='fixed']")
        assert len(joints) == 2

    def test_custom_prefix(self, robot: Any) -> None:
        spec = BarbellSpec.mens_olympic()
        links = create_barbell_links(robot, spec, prefix="bar")
        assert "bar_shaft" in links

    def test_mass_conservation(self, robot: Any) -> None:
