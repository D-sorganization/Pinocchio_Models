...
def _collect_link_names_and_joints(
    ...
    # ⚡ Bolt Optimization: Cache methods to local variables to minimize dictionary lookup overhead
    link_names_add = link_names.add
    joints_append = joints.append

    # ⚡ Bolt Optimization: Prioritize the 'happy path' by checking membership in `_VALID_TAGS`
    # before checking for non-string callables. Python's `in` safely returns False for callables.
    for el in root.iter():
        tag = el.tag
...
def ensure_valid_urdf_tree(root: ET.Element) -> ET.Element:  # noqa: C901
    ...
    for joint in joints:
        ...
        # ⚡ Bolt Optimization: Inline validation logic to avoid Python function
        # call overhead for every node in high-frequency validation loops.
