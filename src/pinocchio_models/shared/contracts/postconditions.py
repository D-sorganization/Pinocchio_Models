...
def ensure_valid_urdf_tree(root: ET.Element) -> ET.Element:  # noqa: C901
    ...
    # ⚡ Bolt Optimization: Cache list/set insertion methods
    add_link_name = link_names.add
    append_joint = joints.append

    # ⚡ Bolt Optimization: Deep single pass iteration, checking valid tags on the happy path
    for el in root.iter():
        tag = el.tag
    ...
    for joint in joints:
        # Avoid .find() call overhead. Elements are small lists of children.
        parent_el = None
        child_el = None
        for child in joint:
            child_tag = child.tag
