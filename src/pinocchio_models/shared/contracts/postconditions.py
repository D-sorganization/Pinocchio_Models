...
def ensure_valid_urdf_tree(root: ET.Element) -> ET.Element:  # noqa: C901
    ...
    add_link_name = link_names.add
    append_joint = joints.append

    # ⚡ Bolt Optimization: Using `in` over string membership before type checking
    # creates a fast-path for valid tags without type check overhead. Python safely returns
    # False if the tag is a callable comment instead of a string.
    for el in root.iter():
        tag = el.tag
    ...
    # ⚡ Bolt Optimization: Removing function overhead by inlining validation,
    # and unrolling the joint parent/child attribute collection
    for joint in joints:
