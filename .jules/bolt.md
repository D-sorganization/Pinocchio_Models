...
## 2026-07-21 - Optimize node collection loops in URDF generation
**Learning:** During profiling of URDF tree generation, the `_collect_link_names_and_joints` function inside `src/pinocchio_models/shared/contracts/postconditions.py` was observed consuming measurable overhead in the tight node iteration loop. Python dictionary method lookup for sets/lists (`link_names.add`, `joints.append`) adds up when evaluated per-node over complex multi-body trees. Additionally, checking if `type(tag) is not str` for every node adds type-check overhead, despite >99% of nodes actually being standard string tags.
**Action:** Caching Python methods into local variables (e.g. `link_names_add = link_names.add`) outside the hot loop avoids dictionary lookups per iteration. Re-ordering validations to check the happy path first (`tag in _VALID_TAGS`) safely bypasses the `type()` check for almost all nodes (since callables like `Comment` will simply safely return `False` for the string set membership check and fall back correctly).
