...
**Learning:** In high-frequency URDF/XML validation paths (e.g. `ensure_valid_urdf_tree`), having a separate helper function (`_collect_link_names_and_joints`) that traverses the ElementTree and returns collections adds unnecessary function call and unpacking overhead. Additionally, using multiple `.find()` calls to retrieve `parent` and `child` links in joints scans the child tags multiple times.
**Action:** Inline small helper functions into a single deep-iteration loop within the main validation function. Cache class methods like `set.add` and `list.append` outside the loop, use the happy path check (`tag in _VALID_TAGS`) before type checking, and iterate directly over joint children (`for child in joint:`) to break early when both `parent` and `child` are found. This reduced the validation time by ~20%.
