...
## 2024-05-24 - Avoid compound `or` in XML escaping and cache `len(elem)`
**Learning:** In Python, compound boolean expressions (like `if "&" in text or "<" in text:`) actually incur a penalty compared to sequential, independent `if` statements when escaping strings that mostly do *not* contain those characters. C-level string operations in Python are extremely fast, but adding the evaluation of multiple conditions in a compound statement slows the happy path down. Additionally, directly evaluating `if not elem:` for checking children is deprecated; cache `len(elem)` into a local variable (`elem_len`) before checking it multiple times in recursive functions.
**Action:** Remove compound `or` pre-checks for string replacements on the hot path in favor of multiple `if` lines, and always cache `len(elem)` early in a recursive ElementTree traversal.
